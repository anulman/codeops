...
function NewSessionPage() {
  ...
  return (
    <AppShell>
      <main {...sx("min-h-[calc(100dvh-52px)] bg-[#111113] px-4 py-8 lg:min-h-dvh lg:px-8 lg:py-12")}>
        <div {...sx("mx-auto max-w-3xl")}>
          <div {...sx("flex items-center justify-between gap-4")}>
            <div>
              <p {...sx("text-[10px] font-semibold uppercase tracking-[0.15em] text-white/28")}>Workspace launch</p>
              <h1 {...sx("mt-2 text-3xl font-semibold tracking-[-0.045em] text-white/92")}>New session</h1>
              <p {...sx("mt-2 max-w-xl text-sm leading-6 text-white/40")}>Give the agent a prompt and choose the source repositories it can work with.</p>
            </div>
            <Link to="/" {...sx("grid size-10 shrink-0 place-items-center rounded-xl border border-white/[0.07] text-lg text-white/35 transition hover:bg-white/[0.04] hover:text-white/70")} aria-label="Close new session">×</Link>
          </div>

          <section {...sx("mt-8 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#171719] shadow-[0_24px_80px_rgba(0,0,0,.22)]")}>
            <div {...sx("p-5 sm:p-7")}>
              <label {...sx("block")}>
                <span {...sx("text-xs font-semibold text-white/72")}>What should the agent do?</span>
                <textarea autoFocus required disabled={locked} value={prompt} onChange={(event) => setPrompt(event.target.value)} maxLength={100_000} rows={7} placeholder="Build, investigate, write, or analyze…" {...sx("mt-3 w-full resize-y rounded-xl border border-white/[0.08] bg-black/15 px-4 py-3 text-sm leading-6 text-white/82 outline-none placeholder:text-white/22 focus:border-[#7774ff]/50 focus:ring-2 focus:ring-[#7774ff]/10 disabled:opacity-55")} />
              </label>

              <label {...sx("mt-6 block")}>
                <span {...sx("text-xs font-semibold text-white/72")}>Title <span {...sx("font-normal text-white/28")}>Optional</span></span>
                <input disabled={locked} value={title} onChange={(event) => setTitle(event.target.value)} maxLength={200} placeholder="A short label for this session" {...sx("mt-3 h-11 w-full rounded-xl border border-white/[0.08] bg-black/15 px-4 text-sm text-white/82 outline-none placeholder:text-white/22 focus:border-[#7774ff]/50 focus:ring-2 focus:ring-[#7774ff]/10 disabled:opacity-55")} />
              </label>
              ...
              <fieldset {...sx("mt-7")} disabled={locked}>
                ...
                <div {...sx("mt-3 grid gap-2 sm:grid-cols-2")}>
                  {sessionModes.map((option) => {
                    ...
                    return <label key={option.id} {...sx(`cursor-pointer rounded-xl border p-3.5 transition ${checked ? "border-[#7774ff]/45 bg-[#7774ff]/10" : "border-white/[0.07] bg-white/[0.02] hover:border-white/[0.13]"}`)}>
                      <span {...sx("flex items-center gap-2")}><input type="radio" name="session-mode" value={option.id} checked={checked} onChange={() => setMode(option.id)} {...sx("sr-only")} /><span {...sx(`size-2 rounded-full ${checked ? "bg-[#8c89ff]" : "bg-white/15"}`)} /><span {...sx("text-sm font-medium text-white/76")}>{option.label}</span></span>
                      <span {...sx("mt-2 block text-[11px] leading-4 text-white/38")}>{option.summary}</span>
                      <span {...sx("mt-2 block font-mono text-[9px] text-white/24")}>{option.policy}</span>
                    </label>;
                  })}
                </div>
                <p {...sx("mt-2 text-[10px] text-white/24")}>Validate is deterministic and uses no model. Interactive launch stays disabled until CodeOps provides a deterministic validation action.</p>
              </fieldset>
              ...
              <fieldset {...sx("mt-7")} disabled={locked}>
                ...
                <div {...sx("mt-3 grid gap-2 sm:grid-cols-2")}>
                  {catalog.repositories.map((repository) => {
                    ...
                    return (
                      <label key={repository.key} {...sx(`relative flex cursor-pointer items-start gap-3 rounded-xl border p-3.5 transition ${checked ? "border-[#7774ff]/45 bg-[#7774ff]/10" : "border-white/[0.07] bg-white/[0.02] hover:border-white/[0.13] hover:bg-white/[0.035]"}`)}>
                        <input type="checkbox" checked={checked} onChange={() => toggleRepository(repository.key)} {...sx("peer sr-only")} />
                        <span {...sx(`mt-0.5 grid size-4 shrink-0 place-items-center rounded border text-[10px] ${checked ? "border-[#8c89ff] bg-[#6d6af7] text-white" : "border-white/20 text-transparent"}`)}>✓</span>
                        <span {...sx("min-w-0")}><span {...sx("block truncate text-sm font-medium text-white/76")}>{repository.label}</span><span {...sx("mt-1 block truncate font-mono text-[10px] text-white/28")}>{repository.repository} · {repository.defaultRef}</span></span>
                      </label>
                    );
                  })}
                </div>
              ...
              <fieldset {...sx("mt-7")} disabled={locked}>
                ...
                {contextAttachments.length > 0 ? <div {...sx("mt-3 space-y-2")}>{contextAttachments.map((attachment) => <div key={attachment.attachmentId} {...sx("flex items-center gap-3 rounded-xl border border-white/[0.07] bg-white/[0.02] px-3 py-2.5")}><span {...sx("grid size-7 shrink-0 place-items-center rounded-lg bg-[#6da8ff]/8 text-[11px] text-[#8dbbff]")}>↗</span><span {...sx("min-w-0 flex-1")}><span {...sx("block truncate text-xs font-medium text-white/66")}>{attachment.name}</span><span {...sx("mt-0.5 block truncate font-mono text-[9px] text-white/25")}>{contextAttachmentSummary(attachment)}</span></span><button type="button" disabled={locked} onClick={() => setContextAttachments((current) => current.filter((item) => item.attachmentId !== attachment.attachmentId))} aria-label={`Remove ${attachment.name}`} {...sx("grid size-8 shrink-0 place-items-center rounded-lg text-white/25 transition hover:bg-white/[0.05] hover:text-white/65")}>×</button></div>)}</div> : null}
              </fieldset>

              <div {...sx(`mt-5 flex items-start gap-3 rounded-xl border px-4 py-3 ${selectedRepositories.length === 0 ? "border-[#6da8ff]/16 bg-[#6da8ff]/7" : "border-white/[0.06] bg-white/[0.02]"}`)}>
                <span {...sx("mt-0.5 text-sm text-[#8dbbff]")}>{selectedRepositories.length === 0 ? "✦" : "↳"}</span>
                <div><p {...sx("text-xs font-medium text-white/62")}>{selectedRepositories.length === 0 ? "Scratch workspace" : `${selectedRepositories.length} source ${selectedRepositories.length === 1 ? "repository" : "repositories"}`}</p><p {...sx("mt-1 text-[11px] leading-4 text-white/30")}>{mode === "implement" ? selectedRepositories.length === 0 ? "The agent starts with an empty bounded writable workspace. Files remain session artifacts until you choose to publish them." : "The agent receives fixed checkouts plus a bounded writable scratch area. Publication always requires a later explicit action." : "The complete workspace is mounted read-only for this mode. The agent can inspect it but cannot change source or scratch files."}</p></div>
              </div>
            </div>

            <div {...sx("flex flex-col gap-3 border-t border-white/[0.07] bg-black/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7")}>
              <div aria-live="polite" {...sx("min-h-5 text-[11px] text-white/34")}>
                {error ? <span role="alert" {...sx("text-[#ff989d]")}>{error}</span> : submitting ? <span {...sx("flex items-center gap-2")}><span {...sx("size-1.5 animate-pulse rounded-full bg-[#6da8ff]")} />Creating the session…</span> : "The initial prompt is delivered exactly once after the session is ready."}
              </div>
              <div {...sx("flex shrink-0 gap-2")}>
                <Link to="/" {...sx("grid h-11 place-items-center rounded-lg px-4 text-xs font-medium text-white/40 transition hover:bg-white/[0.04] hover:text-white/68")}>Cancel</Link>
                <button type="button" disabled={!valid || locked} onClick={() => void submit()} {...sx("h-11 rounded-lg bg-[#6d6af7] px-5 text-xs font-semibold text-white shadow-[0_8px_24px_rgba(73,69,225,.2)] transition hover:bg-[#7c79ff] disabled:cursor-not-allowed disabled:opacity-35")}>{submitting ? "Creating…" : "Create session"}</button>
              </div>
            </div>
          </section>
        </div>
