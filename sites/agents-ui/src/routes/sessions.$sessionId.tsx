      ...
      <main className="min-h-[calc(100dvh-52px)] bg-[#111113] lg:min-h-dvh">
        ...
        <section className="mx-auto max-w-3xl px-4 py-10 sm:px-8 sm:py-14">
          <div className={`rounded-2xl border p-6 sm:p-8 ${failed ? "border-[#ff7278]/18 bg-[#ff7278]/[0.045]" : "border-white/[0.07] bg-[#171719]"}`} aria-live="polite">
            ...
            {launch.contextAttachments.length > 0 ? <ContextAttachmentList attachments={launch.contextAttachments} className="mt-5" /> : null}
    ...
    return (
      <article className="ml-auto max-w-[88%] py-2 sm:max-w-[78%]">
        <div className="rounded-2xl rounded-br-md border border-[#7774ff]/18 bg-[#7774ff]/10 px-4 py-3 text-sm leading-6 text-white/80 shadow-[0_10px_30px_rgba(0,0,0,.12)]">
          ...
          {message.contextAttachments && message.contextAttachments.length > 0 ? <ContextAttachmentList attachments={message.contextAttachments} className="mt-3 border-t border-white/[0.08] pt-3" /> : null}
        </div>
...
function Inspector({ session, relatedSessions, comparisonCandidates }: Readonly<{ session: SessionSnapshot; relatedSessions: readonly SessionSnapshot[]; comparisonCandidates: readonly SessionSnapshot[] }>) {
  ...
  return <div className="space-y-6">
    ...
    {isWorkspaceIdentity(identity) ? <InspectorSection title="Context"><Fact label="Attachments" value={String(identity.contextAttachments.length)} />{identity.contextAttachments.length > 0 ? <ContextAttachmentList attachments={identity.contextAttachments} /> : <p className="text-xs text-white/28">No context files.</p>}</InspectorSection> : null}
    ...
    <InspectorSection title="Ensemble">{relatedSessions.map((item) => <RelatedSession key={item.sessionId} session={item} />)}{relatedSessions.length === 0 ? <p className="text-xs text-white/28">No related sessions.</p> : null}<ForkComparisonPanel target={session} candidates={comparisonCandidates} /></InspectorSection>
...
function ContextAttachmentList({ attachments, className = "" }: Readonly<{ attachments: readonly WorkspaceContextAttachmentDescriptor[]; className?: string }>) {
  return <div className={`space-y-2 ${className}`}>{attachments.map((attachment) => <div key={attachment.attachmentId} className="min-w-0 rounded-lg border border-white/[0.07] bg-black/10 px-3 py-2"><p className="truncate text-[11px] font-medium text-white/58">{attachment.name}</p><p className="mt-0.5 truncate font-mono text-[9px] text-white/25">{contextAttachmentSummary(attachment)}</p></div>)}</div>;
}
