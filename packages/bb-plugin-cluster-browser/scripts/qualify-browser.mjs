...
try {
 ...
 await writeFile('.output/browser-qualification/result.json',JSON.stringify({passed:true,time:new Date().toISOString(),candidateFiles:Object.fromEntries(await Promise.all(['browser.mjs','server.ts','upstream-tools.json'].map(async file=>[file,createHash('sha256').update(await readFile(new URL('../'+file,import.meta.url))).digest('hex')]))),mcp:'0.0.82',playwright:'1.64.0-alpha-1789764292000',screenshotSha256:createHash('sha256').update(bytes).digest('hex'),checks:['two-owner DOM/cookie/localStorage isolation','context retained across calls','fill/click/navigate/press/wait','console/network diagnostics','native PNG delivery','close does not affect peer','reopen clears state']},null,2)+'\n');
