...
const manifestSource = "reviewed Codex login manifest\n";
...
const artifacts = ["codex-login", "codex-smoke", "database", "gateway", "grants", "namespace", "runtime", "ui"]
  .map((id) => ({
    id,
    sha256: createHash("sha256").update(id === "codex-login" ? manifestSource : `${id}\n`).digest("hex"),
  }));
...
const authorization = {
  ...
  authorizedAt: "2026-08-05T18:17:00Z",
};
...
function makeRunner(options = {}) {
  ...
  const runner = (file, args, executionOptions = {}) => {
    ...
    if (file === "kubectl" && args[0] === "-n" && args[2] === "get") {
      const expected = sessionProofApplyResourceIdentities("codex-login").find((resource) =>
      ...
      const uid = options.replaceResourceAfterCreate && round > 0 && expected.kind === "PersistentVolumeClaim"
...
function apply(stub, overrides = {}) {
  return applySessionProofCodexLogin({
    authorization,
    manifestSource,
    startedAt: "2026-08-05T18:18:00Z",
    completedAt: "2026-08-05T18:19:00Z",
    ...overrides,
  }, stub.runner);
}

test("creates only the reviewed Codex login package and receipts four stable server UIDs", () => {
  const stub = makeRunner();
  const result = apply(stub);
  assert.equal(result.receipt.stepId, "codex-login");
...
test("rejects manifest, action, or timestamp drift before any Kubernetes call", () => {
  for (const overrides of [
    ...
    { authorization: { ...authorization, artifact: "codex-smoke" } },
    { startedAt: "2026-08-05T18:16:59Z" },
  ]) {
    const stub = makeRunner();
...
test("refuses any pre-existing Codex login resource before mutation", () => {
  const stub = makeRunner({ preexistingKind: "PersistentVolumeClaim" });
