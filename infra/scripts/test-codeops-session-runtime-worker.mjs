...
test("mounts separate initialization, worker, database, and model authorities", () => {
  ...
  assert.equal(volumes["codex-auth"].persistentVolumeClaim.claimName, "codeops-codex-auth");
  assert.equal(pod.volumes.filter((volume) => volume.persistentVolumeClaim).length, 1);
