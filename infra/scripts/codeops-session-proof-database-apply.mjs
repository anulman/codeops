...
function verifyExecutionTimes(authorization, startedAt, completedAt) {
  if (
    ...
    throw new Error("proof database apply timestamps drifted");
  }
}

function readResource(namespace, expected, runner) {
  const resourceType = RESOURCE_TYPES.get(`${expected.apiVersion}/${expected.kind}`);
  if (!resourceType) throw new Error("proof database resource type is not admitted");
  const source = run([
    "-n", namespace,
  ...
  const resource = parseJson(source, `proof database ${expected.kind}`);
  if (
    ...
    throw new Error("proof database live resource identity drifted");
  }
  return {
...
export function applySessionProofDatabase(input, runner = execFileSync) {
  const authorization = input.authorization;
  verifySessionProofStepAuthorization(authorization);
  if (
    authorization.stepId !== "start-database" ||
    authorization.action !== "operator-apply" ||
    authorization.artifact !== "database"
  ) {
    throw new Error("proof step is not the exact database apply action");
  }
  ...
  if (manifestSha256 !== authorization.artifactSha256) {
    throw new Error("reviewed proof database manifest digest drifted");
  }
  ...
  if (existing.length > 0) {
    throw new Error(`proof database resources already exist: ${existing.map((value) => value.name).join(", ")}`);
  }
  ...
  if (resources.some((resource) => resource === null)) {
    throw new Error("proof database package creation was incomplete");
  }
  ...
  if (
    ...
    throw new Error("proof database resource identity drifted after final identity check");
  }
