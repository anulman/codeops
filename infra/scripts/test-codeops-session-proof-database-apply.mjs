...
const manifestSource = "reviewed database manifest\n";
...
const artifacts = ["codex-login", "codex-smoke", "database", "gateway", "grants", "namespace", "runtime", "ui"]
  .map((id) => ({
    id,
    sha256: createHash("sha256").update(id === "database" ? manifestSource : `${id}\n`).digest("hex"),
  }));
...
const authorization = {
  ...
  authorizedAt: "2026-08-05T18:05:00Z",
};
...
function makeRunner(options = {}) {
  ...
  const runner = (file, args, executionOptions = {}) => {
    ...
    if (file === "kubectl" && args[0] === "-n" && args[2] === "get") {
      const expected = sessionProofApplyResourceIdentities("start-database").find((resource) =>
        typeByIdentity.get(`${resource.apiVersion}/${resource.kind}`) === args[3] && resource.name === args[4]);
      assert.ok(expected);
      if (!created && options.preexistingName !== expected.name) return "";
      if (created) postCreateInventoryReads += 1;
      if (created && options.missingAfterCreate === expected.name) return "";
      const round = Math.floor((postCreateInventoryReads - 1) / 5);
      const uid = options.replaceResourceAfterCreate && round > 0 && expected.name === "codeops-session-proof-database-init"
...
function apply(stub, overrides = {}) {
  return applySessionProofDatabase({
    authorization,
    manifestSource,
    startedAt: "2026-08-05T18:06:00Z",
    completedAt: "2026-08-05T18:07:00Z",
    ...overrides,
  }, stub.runner);
}

test("creates only the reviewed database package and receipts the five stable server UIDs", () => {
  const stub = makeRunner();
  const result = apply(stub);
  assert.equal(result.receipt.stepId, "start-database");
...
test("rejects manifest, action, or timestamp drift before any Kubernetes call", () => {
  for (const overrides of [
    ...
    { startedAt: "2026-08-05T18:04:59Z" },
  ]) {
    const stub = makeRunner();
...
test("refuses any pre-existing package resource before mutation", () => {
  const stub = makeRunner({ preexistingName: "codeops-session-proof-database" });
...
test("withholds a receipt after partial create, missing resources, or identity replacement", () => {
  for (const options of [
    { failCreate: true },
    { missingAfterCreate: "codeops-session-proof-database-init" },
