...
function verifyExecutionTimes(authorization, startedAt, completedAt) {
  if (
    ...
    throw new Error("proof runtime apply timestamps drifted");
  }
}

function readResource(namespace, expected, runner) {
  const resourceType = RESOURCE_TYPES.get(`${expected.apiVersion}/${expected.kind}`);
  if (!resourceType) throw new Error("proof runtime resource type is not admitted");
  const source = run([
    "-n", namespace,
  ...
  const resource = parseJson(source, `proof runtime ${expected.kind}`);
  if (
    ...
    throw new Error("proof runtime live resource identity drifted");
  }
  return {
...
function readInventory(authorization, runner) {
  return sessionProofApplyResourceIdentities(authorization.stepId, authorization).map((expected) =>
    readResource(authorization.namespace.name, expected, runner));
}

export function applySessionProofRuntime(input, runner = execFileSync) {
  const authorization = input.authorization;
  verifySessionProofStepAuthorization(authorization);
  if (
    authorization.stepId !== "start-runtime" ||
    authorization.action !== "operator-apply" ||
    authorization.artifact !== "runtime"
  ) {
    throw new Error("proof step is not the exact runtime apply action");
  }
  ...
  if (manifestSha256 !== authorization.artifactSha256) {
    throw new Error("reviewed proof runtime manifest digest drifted");
  }
  ...
  if (existing.length > 0) {
    throw new Error(`proof runtime resources already exist: ${existing.map((value) => value.name).join(", ")}`);
  }
  ...
  if (resources.some((resource) => resource === null)) {
    throw new Error("proof runtime package creation was incomplete");
  }
  ...
  if (
    ...
    throw new Error("proof runtime resource identity drifted after final identity check");
  }
