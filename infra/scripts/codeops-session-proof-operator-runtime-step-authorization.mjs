...
function assertAuthorizationPath(path, packetPath, namespace, mustBeAbsent) {
  if (!isAbsolute(path ?? "") || resolve(path) !== path) {
    throw new Error("proof fifteenth-step authorization path must be absolute and normalized");
  }
  const parent = dirname(path);
  const parentStat = lstatSync(parent);
  if (
    parent !== dirname(packetPath) ||
    basename(path) !== `${namespace}.step-17-start-runtime.authorization.json` ||
    !parentStat.isDirectory() ||
    parentStat.isSymbolicLink() ||
    realpathSync(parent) !== parent
  ) {
    throw new Error("proof fifteenth-step authorization path must derive exactly from the packet Namespace");
  }
  if (mustBeAbsent) {
    try {
      lstatSync(path);
      throw new Error("proof fifteenth-step authorization already exists");
    } catch (error) {
...
function readPrivateAuthorization(path) {
  ...
  if (
    ...
    throw new Error("proof fifteenth-step authorization must be one bounded private regular file");
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof fifteenth-step authorization changed while it was read");
    }
    return source;
  } finally {
...
function writePrivateAuthorization(path, source) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof fifteenth-step authorization changed while it was written");
    }
  } finally {
    closeSync(descriptor);
  }
...
function buildFifteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  const outputs = readSessionProofUiWaitOutputsFromOperatorPacket(input, runner);
  ...
  const runtimeManifestSource = readSessionProofOperatorArtifact(input, "runtime");
  const authorization = authorizeSessionProofStep({
    ...
    artifactSource: runtimeManifestSource,
    namespaceResource,
    operator,
    target,
    observedAt: input.observedAt,
  });
  return { authorization, uiReadinessOutputs: outputs };
}

export function authorizeFifteenthSessionProofStepFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  return buildFifteenthSessionProofStepAuthorizationFromOperatorPacket(input, runner)
    .authorization;
}

export function persistFifteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  ...
  assertAuthorizationPath(
    input.fifteenthAuthorizationPath,
    input.packetPath,
    namespace,
    true,
  );
  const { authorization } = buildFifteenthSessionProofStepAuthorizationFromOperatorPacket(
    input,
    runner,
  );
  if (authorization.namespace.name !== namespace) {
    throw new Error("proof fifteenth-step authorization Namespace drifted from the operator packet path");
  }
...
export function readFifteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  const packetName = basename(input.packetPath ?? "");
  ...
  const authorizationBytes = readPrivateAuthorization(input.fifteenthAuthorizationPath);
  let authorization;
  try {
    authorization = JSON.parse(authorizationBytes);
  } catch {
    throw new Error("proof fifteenth-step authorization must be valid JSON");
  }
  verifySessionProofStepAuthorization(authorization);
  const { authorization: expected, uiReadinessOutputs } =
    buildFifteenthSessionProofStepAuthorizationFromOperatorPacket({
      ...input,
  ...
  if (!authorizationBytes.equals(Buffer.from(expectedSource))) {
    throw new Error("proof fifteenth-step authorization is not the exact persisted artifact");
  }
  return {
