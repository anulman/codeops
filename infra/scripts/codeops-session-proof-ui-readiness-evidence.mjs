...
function assertAuthorization(authorization) {
  if (
    ...
    throw new Error("proof step is not qualified UI readiness");
  }
}
...
function verifyDeployment(deployment) {
  ...
  if (
    ...
    deployment.name !== "codeops-agents-ui" ||
    ...
    throw new Error("proof UI readiness deployment drifted");
  }
}

export function verifySessionProofUiApplyChain(
  authorization,
  receiptSource,
  applyEvidenceSource,
) {
  if (digest(receiptSource) !== authorization.previousReceiptSha256) {
    throw new Error("proof UI readiness predecessor receipt drifted");
  }
  ...
  if (
    ...
    receipt.stepId !== "start-ui" ||
    receipt.action !== "operator-apply" ||
    receipt.artifact !== "ui" ||
    !SHA256.test(receipt.artifactSha256 ?? "") ||
    receipt.evidenceSha256 !== applyEvidenceSha256
  ) {
    throw new Error("proof UI apply receipt identity drifted");
  }
  const applyEvidence = parseJson(applyEvidenceSource, "proof UI apply evidence");
  ...
  const appliedDeployment = applyEvidence.resourceInventory.find((resource) =>
    ...
    resource.name === "codeops-agents-ui");
  if (!appliedDeployment) {
    throw new Error("proof UI apply evidence has no Deployment identity");
  }
  return appliedDeployment.uid;
}

export function verifySessionProofUiReadinessEvidence(authorization, evidence) {
  ...
  if (
    ...
    typeof evidence.uiApplyReceiptSource !== "string" ||
    Buffer.byteLength(evidence.uiApplyReceiptSource) > MAX_SOURCE_BYTES ||
    typeof evidence.uiApplyEvidenceSource !== "string" ||
    Buffer.byteLength(evidence.uiApplyEvidenceSource) > MAX_SOURCE_BYTES
  ) {
    throw new Error("proof UI readiness evidence identity drifted");
  }
  const appliedDeploymentUid = verifySessionProofUiApplyChain(
    authorization,
    evidence.uiApplyReceiptSource,
    evidence.uiApplyEvidenceSource,
  );
  verifyDeployment(evidence.deployment);
  if (evidence.deployment.uid !== appliedDeploymentUid) {
    throw new Error("proof UI readiness Deployment UID drifted from apply evidence");
  }
  return true;
}

export function buildSessionProofUiReadinessEvidence(input) {
  const authorization = input.authorization;
  assertAuthorization(authorization);
  const receiptSource = input.uiApplyReceiptSource ?? "";
  const applyEvidenceSource = input.uiApplyEvidenceSource ?? "";
  const appliedDeploymentUid = verifySessionProofUiApplyChain(
    authorization,
    receiptSource,
    applyEvidenceSource,
  ...
  const evidence = {
    ...
    namespace: authorization.namespace,
    uiApplyReceiptSource: receiptSource,
