...
function buildRevocationInputs() {
  ...
  for (let stepIndex = 2; stepIndex <= 19; stepIndex += 1) {
    ...
    } else if (step.id === "grant-receipts") {
      evidenceSource = JSON.stringify(buildSessionProofApplyEvidence({
        authorization,
        observedAt: `2026-08-05T18:11:${String(stepIndex).padStart(2, "0")}Z`,
    ...
    } else if (step.id === "wait-codex-smoke") {
      const observedAt = `2026-08-05T18:11:${String(stepIndex).padStart(2, "0")}Z`;
      evidenceSource = JSON.stringify(buildSessionProofCodexSmokeCompletionEvidence({
        authorization,
        smokeReplacementReceiptSource: priorReceiptSources.at(-1),
        smokeReplacementEvidenceSource: priorEvidenceSources.at(-1),
        loginJobAbsent: true,
        job: {
          apiVersion: "batch/v1",
          kind: "Job",
