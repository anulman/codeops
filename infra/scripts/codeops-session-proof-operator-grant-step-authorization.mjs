...
function assertAuthorizationPath(path, packetPath, namespace, mustBeAbsent) {
  if (!isAbsolute(path ?? "") || resolve(path) !== path) {
    throw new Error("proof seventh-step authorization path must be absolute and normalized");
  }
  const parent = dirname(path);
  const parentStat = lstatSync(parent);
  if (
    parent !== dirname(packetPath) ||
    basename(path) !== `${namespace}.step-08-grant-receipts.authorization.json` ||
    !parentStat.isDirectory() ||
    parentStat.isSymbolicLink() ||
    realpathSync(parent) !== parent
  ) {
    throw new Error("proof seventh-step authorization path must derive exactly from the packet Namespace");
  }
  if (mustBeAbsent) {
    try {
      lstatSync(path);
      throw new Error("proof seventh-step authorization already exists");
    } catch (error) {
...
function readPrivateAuthorization(path) {
  ...
  if (
    ...
    throw new Error("proof seventh-step authorization must be one bounded private regular file");
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof seventh-step authorization changed while it was read");
    }
    return source;
  } finally {
...
function writePrivateAuthorization(path, source) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof seventh-step authorization changed while it was written");
    }
  } finally {
    closeSync(descriptor);
  }
...
export function persistSeventhSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  ...
  const authorization = authorizeSeventhSessionProofStepFromOperatorPacket(input, runner);
  if (authorization.namespace.name !== namespace) {
    throw new Error("proof seventh-step authorization Namespace drifted from the operator packet path");
  }
...
export function readSeventhSessionProofStepAuthorizationFromOperatorPacket(
  ...
  const authorizationBytes = readPrivateAuthorization(input.seventhAuthorizationPath);
  let authorization;
  try {
