...
function verifyExecutionTimes(authorization, startedAt, completedAt) {
  if (
    ...
    throw new Error("proof Codex login apply timestamps drifted");
  }
}

function readResource(namespace, expected, runner) {
  const resourceType = RESOURCE_TYPES.get(`${expected.apiVersion}/${expected.kind}`);
  if (!resourceType) throw new Error("proof Codex login resource type is not admitted");
  const source = run([
    "-n", namespace,
  ...
  const resource = parseJson(source, `proof Codex login ${expected.kind}`);
  if (
    ...
    throw new Error("proof Codex login live resource identity drifted");
  }
  return {
...
export function applySessionProofCodexLogin(input, runner = execFileSync) {
  const authorization = input.authorization;
  verifySessionProofStepAuthorization(authorization);
  if (
    authorization.stepId !== "codex-login" ||
    authorization.action !== "operator-apply" ||
    authorization.artifact !== "codex-login"
  ) {
    throw new Error("proof step is not the exact Codex login apply action");
  }
  ...
  if (manifestSha256 !== authorization.artifactSha256) {
    throw new Error("reviewed proof Codex login manifest digest drifted");
  }
  ...
  if (existing.length > 0) {
    throw new Error(`proof Codex login resources already exist: ${existing.map((value) => value.name).join(", ")}`);
  }
  ...
  if (resources.some((resource) => resource === null)) {
    throw new Error("proof Codex login package creation was incomplete");
  }
  ...
  if (
    ...
    throw new Error("proof Codex login resource identity drifted after final identity check");
  }
