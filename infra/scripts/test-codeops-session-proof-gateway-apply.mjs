...
const manifestSource = "reviewed gateway manifest\n";
...
const artifacts = ["codex-login", "codex-smoke", "database", "gateway", "grants", "namespace", "runtime", "ui"]
  .map((id) => ({
    id,
    sha256: createHash("sha256").update(id === "gateway" ? manifestSource : `${id}\n`).digest("hex"),
  }));
...
const authorization = {
  ...
  authorizedAt: "2026-08-05T18:09:00Z",
};
...
function makeRunner(options = {}) {
  ...
  const runner = (file, args, executionOptions = {}) => {
    ...
    if (file === "kubectl" && args[0] === "-n" && args[2] === "get") {
      const expected = sessionProofApplyResourceIdentities("start-gateway").find((resource) =>
...
function apply(stub, overrides = {}) {
  return applySessionProofGateway({
    authorization,
    manifestSource,
    startedAt: "2026-08-05T18:10:00Z",
    completedAt: "2026-08-05T18:11:00Z",
    ...overrides,
  }, stub.runner);
}

test("creates only the reviewed gateway package and receipts four stable server UIDs", () => {
  const stub = makeRunner();
  const result = apply(stub);
  assert.equal(result.receipt.stepId, "start-gateway");
...
test("rejects manifest, action, or timestamp drift before any Kubernetes call", () => {
  for (const overrides of [
    ...
    { authorization: { ...authorization, artifact: "database" } },
    { startedAt: "2026-08-05T18:08:59Z" },
  ]) {
    const stub = makeRunner();
...
test("refuses any pre-existing gateway resource before mutation", () => {
