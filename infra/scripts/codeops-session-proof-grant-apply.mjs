...
function verifyExecutionTimes(authorization, startedAt, completedAt) {
  if (
    ...
    throw new Error("proof grant apply timestamps drifted");
  }
}

function readResource(namespace, expected, runner) {
  const resourceType = RESOURCE_TYPES.get(`${expected.apiVersion}/${expected.kind}`);
  if (!resourceType) throw new Error("proof grant resource type is not admitted");
  const source = run([
    "-n", namespace,
  ...
  const resource = parseJson(source, `proof grant ${expected.kind}`);
  if (
    ...
    throw new Error("proof grant live resource identity drifted");
  }
  return {
...
export function applySessionProofGrants(input, runner = execFileSync) {
  const authorization = input.authorization;
  verifySessionProofStepAuthorization(authorization);
  if (
    authorization.stepId !== "grant-receipts" ||
    authorization.action !== "operator-apply" ||
    authorization.artifact !== "grants"
  ) {
    throw new Error("proof step is not the exact receipt grant apply action");
  }
  ...
  if (manifestSha256 !== authorization.artifactSha256) {
    throw new Error("reviewed proof grant manifest digest drifted");
  }
  ...
  if (existing.length > 0) {
    throw new Error(`proof grant resources already exist: ${existing.map((value) => value.name).join(", ")}`);
  }
  ...
  if (resources.some((resource) => resource === null)) {
    throw new Error("proof grant package creation was incomplete");
  }
  ...
  if (
    ...
    throw new Error("proof grant resource identity drifted after final identity check");
  }
