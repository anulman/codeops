...
function assertAuthorizationPath(path, packetPath, namespace, mustBeAbsent) {
  if (!isAbsolute(path ?? "") || resolve(path) !== path) {
    throw new Error("proof eighteenth-step authorization path must be absolute and normalized");
  }
  const parent = dirname(path);
  const parentStat = lstatSync(parent);
  if (
    parent !== dirname(packetPath) ||
    basename(path) !== `${namespace}.step-22-stop-runtime.authorization.json` ||
    !parentStat.isDirectory() ||
    parentStat.isSymbolicLink() ||
    realpathSync(parent) !== parent
  ) {
    throw new Error("proof eighteenth-step authorization path must derive exactly from the packet Namespace");
  }
  if (mustBeAbsent) {
    try {
      lstatSync(path);
      throw new Error("proof eighteenth-step authorization already exists");
    } catch (error) {
...
function readPrivateAuthorization(path) {
  ...
  if (
    ...
    throw new Error("proof eighteenth-step authorization must be one bounded private regular file");
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof eighteenth-step authorization changed while it was read");
    }
    return source;
  } finally {
...
function writePrivateAuthorization(path, source) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof eighteenth-step authorization changed while it was written");
    }
  } finally {
    closeSync(descriptor);
  }
...
function buildEighteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
  ...
  return { authorization, recordingOutputs: outputs };
}
...
export function authorizeEighteenthSessionProofStepFromOperatorPacket(
  ...
  return buildEighteenthSessionProofStepAuthorizationFromOperatorPacket(
    input,
    runner,
    readRecordingOutputs,
    authorizeStep,
  ).authorization;
}

export function persistEighteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
  authorizeStep = authorizeEighteenthSessionProofStepFromOperatorPacket,
) {
  ...
  assertAuthorizationPath(
    input.eighteenthAuthorizationPath,
    input.packetPath,
    namespace,
    true,
  );
  ...
  if (authorization.namespace.name !== namespace) {
    throw new Error("proof eighteenth-step authorization Namespace drifted from the operator packet path");
  }
...
export function readEighteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
  buildAuthorization = buildEighteenthSessionProofStepAuthorizationFromOperatorPacket,
) {
  const packetName = basename(input.packetPath ?? "");
  const namespace = packetName.endsWith(".packet")
    ? packetName.slice(0, -".packet".length)
  ...
  const authorizationBytes = readPrivateAuthorization(input.eighteenthAuthorizationPath);
  let authorization;
  try {
    authorization = JSON.parse(authorizationBytes);
  } catch {
    throw new Error("proof eighteenth-step authorization must be valid JSON");
  }
  verifySessionProofStepAuthorization(authorization);
  const { authorization: expected, recordingOutputs } = buildAuthorization({
    ...input,
  ...
  if (!authorizationBytes.equals(Buffer.from(expectedSource))) {
    throw new Error("proof eighteenth-step authorization is not the exact persisted artifact");
  }
  return {
