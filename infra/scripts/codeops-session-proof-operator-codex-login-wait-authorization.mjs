...
function assertAuthorizationPath(path, packetPath, namespace, mustBeAbsent) {
  if (!isAbsolute(path ?? "") || resolve(path) !== path) {
    throw new Error("proof tenth-step authorization path must be absolute and normalized");
  }
  const parent = dirname(path);
  const parentStat = lstatSync(parent);
  if (
    parent !== dirname(packetPath) ||
    basename(path) !== `${namespace}.step-11-wait-codex-login.authorization.json` ||
    !parentStat.isDirectory() ||
    parentStat.isSymbolicLink() ||
    realpathSync(parent) !== parent
  ) {
    throw new Error("proof tenth-step authorization path must derive exactly from the packet Namespace");
  }
  if (mustBeAbsent) {
    try {
      lstatSync(path);
      throw new Error("proof tenth-step authorization already exists");
    } catch (error) {
...
function readPrivateAuthorization(path) {
  ...
  if (
    ...
    throw new Error("proof tenth-step authorization must be one bounded private regular file");
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof tenth-step authorization changed while it was read");
    }
    return source;
  } finally {
...
function writePrivateAuthorization(path, source) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof tenth-step authorization changed while it was written");
    }
  } finally {
    closeSync(descriptor);
  }
...
export function persistTenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  ...
  assertAuthorizationPath(
    input.tenthAuthorizationPath,
    input.packetPath,
    namespace,
    true,
  );
  const authorization = authorizeTenthSessionProofStepFromOperatorPacket(input, runner);
  if (authorization.namespace.name !== namespace) {
    throw new Error("proof tenth-step authorization Namespace drifted from the operator packet path");
  }
...
export function readTenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  const outputs = readSessionProofCodexLoginApplyOutputsFromOperatorPacket(input, runner);
  ...
  const authorizationBytes = readPrivateAuthorization(input.tenthAuthorizationPath);
  let authorization;
  try {
    authorization = JSON.parse(authorizationBytes);
  } catch {
    throw new Error("proof tenth-step authorization must be valid JSON");
  }
  const expected = authorizeTenthSessionProofStepFromOperatorPacket({
    ...input,
  ...
  if (!authorizationBytes.equals(Buffer.from(expectedSource))) {
    throw new Error("proof tenth-step authorization is not the exact persisted artifact");
  }
