...
function assertAuthorizationPath(path, packetPath, namespace, mustBeAbsent) {
  if (!isAbsolute(path ?? "") || resolve(path) !== path) {
    throw new Error("proof fourteenth-step authorization path must be absolute and normalized");
  }
  const parent = dirname(path);
  const parentStat = lstatSync(parent);
  if (
    parent !== dirname(packetPath) ||
    basename(path) !== `${namespace}.step-16-wait-ui.authorization.json` ||
    !parentStat.isDirectory() ||
    parentStat.isSymbolicLink() ||
    realpathSync(parent) !== parent
  ) {
    throw new Error("proof fourteenth-step authorization path must derive exactly from the packet Namespace");
  }
  if (mustBeAbsent) {
    try {
      lstatSync(path);
      throw new Error("proof fourteenth-step authorization already exists");
    } catch (error) {
...
function readPrivateAuthorization(path) {
  ...
  if (
    ...
    throw new Error("proof fourteenth-step authorization must be one bounded private regular file");
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof fourteenth-step authorization changed while it was read");
    }
    return source;
  } finally {
...
function writePrivateAuthorization(path, source) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof fourteenth-step authorization changed while it was written");
    }
  } finally {
    closeSync(descriptor);
  }
...
function buildFourteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  const outputs = readSessionProofUiApplyOutputsFromOperatorPacket(input, runner);
  ...
  return { authorization, uiApplyOutputs: outputs };
}

export function authorizeFourteenthSessionProofStepFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  return buildFourteenthSessionProofStepAuthorizationFromOperatorPacket(input, runner)
    .authorization;
}

export function persistFourteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  ...
  assertAuthorizationPath(
    input.fourteenthAuthorizationPath,
    input.packetPath,
    namespace,
    true,
  );
  const { authorization } = buildFourteenthSessionProofStepAuthorizationFromOperatorPacket(
    input,
    runner,
  );
  if (authorization.namespace.name !== namespace) {
    throw new Error("proof fourteenth-step authorization Namespace drifted from the operator packet path");
  }
...
export function readFourteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  const packetName = basename(input.packetPath ?? "");
  ...
  const authorizationBytes = readPrivateAuthorization(input.fourteenthAuthorizationPath);
  let authorization;
  try {
    authorization = JSON.parse(authorizationBytes);
  } catch {
    throw new Error("proof fourteenth-step authorization must be valid JSON");
  }
  verifySessionProofStepAuthorization(authorization);
  const { authorization: expected, uiApplyOutputs } =
    buildFourteenthSessionProofStepAuthorizationFromOperatorPacket({
      ...input,
  ...
  if (!authorizationBytes.equals(Buffer.from(expectedSource))) {
    throw new Error("proof fourteenth-step authorization is not the exact persisted artifact");
  }
  return {
