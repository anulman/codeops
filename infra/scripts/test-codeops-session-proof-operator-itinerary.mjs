...
test("wires every closed proof step and exact byte source without invoking an adapter", () => {
  ...
  assert.deepEqual(
    result.steps.find((step) => step.stepId === "start-gateway").adapter,
    {
      ...
      export: "persistSessionProofGatewayApplyFromOperatorPacket",
    },
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    result.steps.find((step) => step.stepId === "start-runtime").adapter,
    {
      module: "./codeops-session-proof-operator-runtime-apply.mjs",
  ...
  assert.deepEqual(
    result.steps.find((step) => step.stepId === "record-proof").adapter,
    {
      ...
      export: "persistSessionProofRecordingFromOperatorPacket",
    },
  );
  assert.deepEqual(
