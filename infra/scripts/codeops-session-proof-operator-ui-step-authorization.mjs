...
function assertAuthorizationPath(path, packetPath, namespace, mustBeAbsent) {
  if (!isAbsolute(path ?? "") || resolve(path) !== path) {
    throw new Error("proof thirteenth-step authorization path must be absolute and normalized");
  }
  const parent = dirname(path);
  const parentStat = lstatSync(parent);
  if (
    parent !== dirname(packetPath) ||
    basename(path) !== `${namespace}.step-14-start-ui.authorization.json` ||
    !parentStat.isDirectory() ||
    parentStat.isSymbolicLink() ||
    realpathSync(parent) !== parent
  ) {
    throw new Error("proof thirteenth-step authorization path must derive exactly from the packet Namespace");
  }
  if (mustBeAbsent) {
    try {
      lstatSync(path);
      throw new Error("proof thirteenth-step authorization already exists");
    } catch (error) {
...
function readPrivateAuthorization(path) {
  ...
  if (
    ...
    throw new Error("proof thirteenth-step authorization must be one bounded private regular file");
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof thirteenth-step authorization changed while it was read");
    }
    return source;
  } finally {
...
function writePrivateAuthorization(path, source) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof thirteenth-step authorization changed while it was written");
    }
  } finally {
    closeSync(descriptor);
  }
...
function buildThirteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  ...
  return { authorization, completionOutputs: outputs };
}
...
export function authorizeThirteenthSessionProofStepFromOperatorPacket(
  ...
  return buildThirteenthSessionProofStepAuthorizationFromOperatorPacket(input, runner)
    .authorization;
}

export function persistThirteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  ...
  assertAuthorizationPath(
    input.thirteenthAuthorizationPath,
    input.packetPath,
    namespace,
    true,
  );
  const { authorization } = buildThirteenthSessionProofStepAuthorizationFromOperatorPacket(
    input,
    runner,
  );
  if (authorization.namespace.name !== namespace) {
    throw new Error("proof thirteenth-step authorization Namespace drifted from the operator packet path");
  }
...
export function readThirteenthSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  const packetName = basename(input.packetPath ?? "");
  ...
  const authorizationBytes = readPrivateAuthorization(input.thirteenthAuthorizationPath);
  let authorization;
  try {
    authorization = JSON.parse(authorizationBytes);
  } catch {
    throw new Error("proof thirteenth-step authorization must be valid JSON");
  }
  verifySessionProofStepAuthorization(authorization);
  const { authorization: expected, completionOutputs } =
    buildThirteenthSessionProofStepAuthorizationFromOperatorPacket({
      ...input,
  ...
  if (!authorizationBytes.equals(Buffer.from(expectedSource))) {
    throw new Error("proof thirteenth-step authorization is not the exact persisted artifact");
  }
  return {
