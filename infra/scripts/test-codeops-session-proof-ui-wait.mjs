...
const applyEvidenceSource = JSON.stringify(buildSessionProofApplyEvidence({
  ...
  observedAt: "2026-08-05T21:06:00Z",
  resources: sessionProofApplyResourceIdentities("start-ui").map((resource, index) => ({
    ...resource,
    uid: resource.kind === "Deployment" ? "ui-deployment-uid" : `ui-resource-uid-${index}`,
  })),
}));
...
const authorization = {
  ...
  authorizedAt: "2026-08-05T21:07:00Z",
};
...
function deployment({
  ...
  return {
    apiVersion: "apps/v1",
    kind: "Deployment",
    metadata: { name: "codeops-agents-ui", namespace, uid, generation },
    spec: { replicas: 1 },
    status: {
...
function wait(stub, overrides = {}, waitCalls = []) {
  return waitForSessionProofUi({
    authorization,
    uiApplyReceiptSource: applyReceiptSource,
    uiApplyEvidenceSource: applyEvidenceSource,
    startedAt: "2026-08-05T21:08:00Z",
    completedAt: "2026-08-05T21:09:00Z",
    maxAttempts: 3,
    pollIntervalMs: 1000,
    ...overrides,
...
test("polls only the exact UI Deployment and receipts its stable ready identity", () => {
  const stub = makeRunner([deployment({ ready: false }), deployment(), deployment()]);
  const waits = [];
  const result = wait(stub, {}, waits);
  assert.equal(result.receipt.stepId, "wait-ui");
  assert.equal(result.receipt.result, "completed");
  assert.equal(JSON.parse(result.evidenceSource).deployment.uid, "ui-deployment-uid");
...
test("rejects action, time, chain, or polling-bound drift before Kubernetes access", () => {
  for (const overrides of [
    ...
    { startedAt: "2026-08-05T21:06:59Z" },
    { maxAttempts: 121 },
...
test("withholds completion on timeout within the reviewed attempt bound", () => {
