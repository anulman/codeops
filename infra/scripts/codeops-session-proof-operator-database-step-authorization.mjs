...
function assertAuthorizationPath(path, packetPath, namespace, mustBeAbsent) {
  if (!isAbsolute(path ?? "") || resolve(path) !== path) {
    throw new Error("proof third-step authorization path must be absolute and normalized");
  }
  const parent = dirname(path);
  const parentStat = lstatSync(parent);
  if (
    parent !== dirname(packetPath) ||
    basename(path) !== `${namespace}.step-04-start-database.authorization.json` ||
    !parentStat.isDirectory() ||
    parentStat.isSymbolicLink() ||
    realpathSync(parent) !== parent
  ) {
    throw new Error("proof third-step authorization path must derive exactly from the packet Namespace");
  }
  if (mustBeAbsent) {
    try {
      lstatSync(path);
      throw new Error("proof third-step authorization already exists");
    } catch (error) {
...
function readPrivateAuthorization(path) {
  ...
  if (
    ...
    throw new Error("proof third-step authorization must be one bounded private regular file");
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof third-step authorization changed while it was read");
    }
    return source;
  } finally {
...
function writePrivateAuthorization(path, source) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("proof third-step authorization changed while it was written");
    }
  } finally {
    closeSync(descriptor);
  }
...
export function persistThirdSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  ...
  assertAuthorizationPath(
    input.thirdAuthorizationPath,
    input.packetPath,
    namespace,
    true,
  );
  const outputs = readSecondSessionProofCredentialOutputsFromOperatorPacket(input, runner);
  if (outputs.creationReceipt.namespace.name !== namespace) {
    throw new Error("proof third-step authorization Namespace drifted from the operator packet path");
  }
  const authorization = authorizeThirdSessionProofStepFromOperatorPacket(input, runner);
...
export function readThirdSessionProofStepAuthorizationFromOperatorPacket(
  input,
  runner = execFileSync,
) {
  const outputs = readSecondSessionProofCredentialOutputsFromOperatorPacket(input, runner);
  ...
  const authorizationBytes = readPrivateAuthorization(input.thirdAuthorizationPath);
  let authorization;
  try {
    authorization = JSON.parse(authorizationBytes);
  } catch {
    throw new Error("proof third-step authorization must be valid JSON");
  }
  const expected = authorizeThirdSessionProofStepFromOperatorPacket({
    ...input,
  ...
  if (!authorizationBytes.equals(Buffer.from(expectedSource))) {
    throw new Error("proof third-step authorization is not the exact persisted artifact");
  }
