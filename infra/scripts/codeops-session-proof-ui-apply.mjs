...
function verifyExecutionTimes(authorization, startedAt, completedAt) {
  if (
    ...
    throw new Error("proof UI apply timestamps drifted");
  }
}

function readResource(namespace, expected, runner) {
  const resourceType = RESOURCE_TYPES.get(`${expected.apiVersion}/${expected.kind}`);
  if (!resourceType) throw new Error("proof UI resource type is not admitted");
  const source = run([
    "-n", namespace,
  ...
  const resource = parseJson(source, `proof UI ${expected.kind}`);
  if (
    ...
    throw new Error("proof UI live resource identity drifted");
  }
  return {
...
export function applySessionProofUi(input, runner = execFileSync) {
  const authorization = input.authorization;
  verifySessionProofStepAuthorization(authorization);
  if (
    authorization.stepId !== "start-ui" ||
    authorization.action !== "operator-apply" ||
    authorization.artifact !== "ui"
  ) {
    throw new Error("proof step is not the exact UI apply action");
  }
  ...
  if (manifestSha256 !== authorization.artifactSha256) {
    throw new Error("reviewed proof UI manifest digest drifted");
  }
  ...
  if (existing.length > 0) {
    throw new Error(`proof UI resources already exist: ${existing.map((value) => value.name).join(", ")}`);
  }
  ...
  if (resources.some((resource) => resource === null)) {
    throw new Error("proof UI package creation was incomplete");
  }
  ...
  if (
    ...
    throw new Error("proof UI resource identity drifted after final identity check");
  }
