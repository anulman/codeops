...
`step-04-start-database.authorization.json`, fsyncs and identity-checks its
bytes and parent, and reconstructs the exact authorization from the private
predecessor chain on readback. A substituted or existing target fails before
...
The next read-only handoff reopens those exact private step-4 outputs through
stable no-follow descriptors, re-verifies the persisted database authorization
and current principal, target, and Namespace UID, validates the specialized
five-resource apply evidence, reconstructs the canonical completion receipt
from the exact evidence bytes, and verifies its predecessor and hash chain.
Only `wait-database` can then be authorized, with no manifest input. This
...
Its persistence boundary requires the exact adjacent absent Namespace-derived
path before live reads, then exclusively creates the canonical authorization as
mode `0600`, fsyncs and identity-checks its bytes and parent, and reconstructs
the exact artifact from the private gateway apply chain on readback. A
...
The bounded-waiter handoff then reopens and reconstructs that exact private
authorization, re-verifies the specialized four-resource grant apply evidence
and its canonical completion receipt, and passes only those exact predecessor
bytes plus reviewed polling bounds and bounded execution timestamps to the
existing grant waiter. Authorization, predecessor, permission, identity, or
...
Its persistence boundary requires the exact adjacent absent Namespace-derived
`step-10-codex-login.authorization.json` path before live reads, then
exclusively creates the canonical authorization as mode `0600`, fsyncs and
identity-checks its bytes and parent, and reconstructs the exact artifact from
the private grant-completion chain on readback. A substituted or existing
...
Its persistence boundary requires the exact adjacent absent Namespace-derived
`step-11-wait-codex-login.authorization.json` path before live reads, then
exclusively creates the canonical authorization as mode `0600`, fsyncs and
identity-checks its bytes and parent, and reconstructs the exact artifact from
the private login-apply chain on readback. A substituted or existing target
fails before live access. It imports and invokes no waiter and remains
unreferenced by runners and workflows.

The bounded-waiter handoff then reopens and reconstructs that exact private
authorization, re-verifies the specialized four-resource login apply evidence
and its canonical completion receipt, and passes only those exact predecessor
bytes plus reviewed polling bounds and bounded execution timestamps to the
existing login waiter. Authorization, predecessor, permission, identity, or
output drift fails before the waiter can be reached. No runner or workflow
invokes this operator boundary.

Its durable output boundary exclusively reserves the exact Namespace-derived
adjacent mode-`0600` step-11 evidence and completion-receipt files and fsyncs
their parent before the waiter can be reached. After successful bounded login
completion it fsyncs and identity-checks the specialized evidence and its hash-
bound canonical receipt, then fsyncs the parent again. Readback reconstructs
the exact result from the private authorization and login apply chain. Existing
or substituted outputs and interrupted writes fail closed. No runner or
workflow invokes this persistence boundary.
