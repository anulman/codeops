...
export class SocketAcpWorkspaceLifecycle implements AcpWorkspaceLifecycle {
  ...
  async prompt(dispatch: PromptDispatch): Promise<RuntimeExecutionResult> {
    ...
    try {
      ...
      return { type: "prompt", material: {
        response: `Budget ${stop}. Work is checkpointed and resumable from checkpoint ${"descriptor" in checkpoint.material ? checkpoint.material.descriptor.manifest.checkpointId : checkpoint.material.checkpointId}. Validation and staging are pending unless recorded in the saved work. No completion of the feature or merge is claimed.`,
